<template>
    <div>
        <form class=" max-w-2xl mx-auto my-9 px-10 register" @submit.prevent.stop="handleSubmit">
            <div class="admin flex justify-between gap-3">
                <div class="logo">
                    <fa icon="user-plus" class=" text-2xl p-2 shadow-2xl border border-gray-200 align-middle" />
                </div>
                <h1 class=" relative -top-2 font-bold text-lg bg-green-0 text-green-900 py-4 px-4 rounded shadow-sm">
                    Register as Admin in GROUP 5 - DBMS <p
                        class=" text-xs inline-block align-middle relative -top-0.5 font-semibold"> [
                        <fa icon="user-lock" class="" /> PRISON RECORD ]
                    </p>
                </h1>
            </div>
            <label for="name" class="text-sm font-bold text-green-900 align-middle">Name
                <fa icon="user" />
            </label>
            <br>
            <input type="text" name="yname" id="yname" placeholder="Enter your name" v-model="yname"
            ...
            <br>
            <label for="email" class="text-sm font-bold text-green-900 align-middle">Email
                <fa icon="envelope" />
            </label>
            ...
            <br>
            <label for="password" class="text-green-900 font-bold tracking-norm">Password
                <fa icon="key" /> [ StaffID ]
            </label>
            <div class="genpass flex gap-1 relative">
                <input type="password" placeholder="put your password" v-model="password" ref="pass"
                ...
                <button @click.prevent="generatePassword">
                    <fa icon="fingerprint" class="shadow-2xl p-2 rounded-2xl h-6 text-green-900 relative top-1" />
                </button>
            </div>
            <br>
            <label for="cpassword" class="text-green-900 font-bold">Comfirm Password
                <fa icon="check" />
            </label>
            <input type="password" placeholder="comfirm your password" v-model="cpassword" ref="cref" @keyup="cnPassword"
            ...
            <button
                ...
                @click.prevent.stop="handleSubmit">Register
...
export default {
    data() {
        return {
            ...
            cpassword: "comfirm your password...",
    ...
    methods: {
        ...
        handleSubmit() {
            if ((this.password == this.cpassword) && (this.yname.length >= 2) && (this.email.length >= 4) && (this.password.length >= 4)) {
                console.log("submit")
        ...
        generatePassword() {
            this.password = this.yname.substring(0) + "$" + this.yname.substring(1, 1) + this.generatedPassword + "#"
            this.password = this.yname.substring(0, 2) + "#" + this.yname.substring(2, 1) + Math.floor(Math.random() * 1000000) + "$"
